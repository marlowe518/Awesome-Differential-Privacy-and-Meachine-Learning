...
if not args.keep_old:  # Keep the running script or not
    # Backup python files.
    copy('seal_link_pred.py', args.res_dir)  # Save the running script to the results directory
...
if args.dataset.startswith('ogbl'):
    dataset = PygLinkPropPredDataset(name=args.dataset)  # torch.data.Dataset
    split_edge = dataset.get_edge_split()  # load from file, Dict[Dict["edge":torch.Tensor, "weight":torch.Tensor, "year":torch.Tensor], Dict["edge":torch.Tensor, "weight":torch.Tensor, "year":torch.Tensor, "edge_neg":torch.Tensor], Dict]
    data = dataset[0] # Whole dataset when attr slice is None
...
if args.dataset.startswith('ogbl-citation'):
    ...
    args.eval_metric = 'hits'  # Evaluating the results by hit ratio.
    directed = False
