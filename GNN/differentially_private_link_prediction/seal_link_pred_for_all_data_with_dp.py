...
parser.add_argument('--uniq_appendix', type=str, default="_20231117")
...
# parser.add_argument('--neighborhood_subgraph,', type=bool, default=True)
...
class SEALDatasetSmall(InMemoryDataset):
    def __init__(self, root, A_csc, split_edge, num_hops, node_features=None, percent=100,
                 ...
                 max_nodes_per_hop=None, directed=False, data=None):
        """
        :param root:
        ...
        node_nums = torch.max(split_edge["train"]["edge"]).item() + 1
        self.train_mat_shape = (node_nums, node_nums)
    ...
    def process(self):
        ...
        if args.data_name.startswith('ogbl'):
            # get pos and neg edges without sampling
            pos_edge, neg_edge = get_pos_neg_edges(self.split, self.split_edge,
                                                   self.data.edge_index, self.data.num_nodes,
                                                   100)  # get pos neg edges without sampling
        else:
        ...
        if self.split == "train":
            ...
            degree_constrained_csc_mat = ssp.csc_matrix((np.ones(len(pos_edge_degree_constrained[0])),
                                                        ...
                                                        shape=self.train_mat_shape, dtype=int)
        ...
        if self.percent is not None:
            pos_edge = get_edge_sampled(pos_edge, self.percent)
            neg_edge = get_edge_sampled(neg_edge, self.percent)

        # Extract enclosing subgraphs for pos and neg edges
        print(f"Extract subgraph with {args.extract_parallel_workers_num} workers")
        if args.extract_parallel_workers_num == 0:
            pos_list = extract_enclosing_subgraphs(
                [0, pos_edge, A_csr, self.node_features, 1, self.num_hops, self.node_label,
                 self.ratio_per_hop, self.max_nodes_per_hop, self.directed, self.A_csc, self.neighborhood_subgraph])
            neg_list = extract_enclosing_subgraphs(
                [0, neg_edge, A_csr, self.node_features, 0, self.num_hops, self.node_label,
                 self.ratio_per_hop, self.max_nodes_per_hop, self.directed, self.A_csc, self.neighborhood_subgraph])
        else:
            pos_list = extract_enclosing_subgraphs_parallel(pos_edge, A_csr, self.node_features, 1, self.num_hops,
                                                            self.node_label,
                                                            self.ratio_per_hop, self.max_nodes_per_hop, self.directed,
                                                            self.A_csc, self.neighborhood_subgraph,
                                                            workers=args.extract_parallel_workers_num)
            neg_list = extract_enclosing_subgraphs_parallel(neg_edge, A_csr, self.node_features, 0, self.num_hops,
                                                            self.node_label,
                                                            self.ratio_per_hop, self.max_nodes_per_hop, self.directed,
                                                            self.A_csc, self.neighborhood_subgraph,
                                                            workers=args.extract_parallel_workers_num)
...
# Testing neighborhood subgraph results
...
def main():
    ...
    if args.data_name.startswith('ogbl'):
        ...
        data = dataset[0]  # only in the ogb dataset, the data should be used
        A_csc = None  # ogb dataset does not have A_csc at the beginning.
    else:
