...
def Plotings(df_compare, markers, metric_name="NDCG@10", x_label=r'${\rm \epsilon}$'):
    ...
    for i in range(1, len(df_compare.columns)-1, 2):
        plt.errorbar(df_compare['epsilon'], df_compare.iloc[:, i], df_compare.iloc[:, i + 1], label=names[i],
...
def load_data_v2(compare_res_file_list, ground_truth: float):
    from functools import reduce
    res_df_list = []
    for file_path in compare_res_file_list:
        res_df = pd.read_csv(file_path, index_col=0, header=0)
        temp = res_df[["epsilon", "final_test", "test_std"]]
        res_df_list.append(temp)
    # comparison_df = pd.merge(res_df_list, on="epsilon", how="inner")
    df_merged = reduce(lambda left, right: pd.merge(left, right, on=['epsilon'],
                                                    how='outer'), res_df_list)
    df_merged.columns = ["epsilon", "DPLP-PS", "DPLP-PS-STD", "DPLP-NP", "DPLP-NP-STD"]
    df_merged["GroundTruth"] = ground_truth
    return df_merged
...
if __name__ == '__main__':
    ...
    compare_res_file_list = ["./results/all_results_20230811011100.csv",
                             "./results/all_results_20230811012141.csv"
                             ]  # PB
    df_compare = load_data_v2(compare_res_file_list, 93.)
