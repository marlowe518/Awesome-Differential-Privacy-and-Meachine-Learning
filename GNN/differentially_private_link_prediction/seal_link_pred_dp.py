...
if __name__ == "__main__":
    for run in range(args.runs):
        ...
        # print("errors from module validation", errors[-5:])
        # model = ModuleValidator.fix(model)
        # errors = ModuleValidator.validate(model, strict=False)
        # print("errors after module fix", errors[-5:])

        # Differentially Private
        from opacus import PrivacyEngine
        privact_engine = PrivacyEngine(accountant="rdp")
        model, optimizer, train_loader = privact_engine.make_private_with_epsilon(
            module=model,
            optimizer=optimizer,
        ...
        for epoch in range(start_epoch, start_epoch + args.epochs):
            ...
            epsilon = privacy_engine.get_epsilon(0.001)
            print(f"epsilon:{epsilon}, delta:{0.001}")
