...
def extract_enclosing_subgraphs_parallel(link_index, A, x, y, num_hops, node_label='drnl',
    ...
    #     outputs = p.map(extract_enclosing_subgraphs, process_args) # OPTION 1
    # with Pool(cpu_worker_num) as p:
    #     outputs = p.map(extract_enclosing_subgraphs, process_args) # OPTION 2
    # result_list = []
...
def get_edge_sampled(edge, percent):
    np.random.seed(123)
    num_pos = edge.size(1)
